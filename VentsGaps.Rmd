...
```
#Map of Sampling Stations in the ECS 
...
We chose not to apply a prevalence filter because we are interested in rare ASVs in deep water masses and at hydrothermal vents. 
...
bt$table
```
The SCM is significantly higher than all other sample types, which are not significantly different from eachother. 
...
###Ordination for sample subgroups 
...
pSurf<-plot_ordination(physeqSurf, ordu, color="Depth", shape="filter")+theme_bw() +scale_color_manual(values= c(ap[4]))+ geom_point(size=3) +ggtitle("Surface") +theme(text = element_text(size=16))
pSurf2 <- pSurf + theme(legend.position="none")
pSurf
```
...
pSCM2<- pSCM + theme(legend.position="none")
pSCM
```
...
pMid2 <- pMid  +  theme(legend.position="None")
pMid2
```
...
pBottom2 <- pBottom+ theme(legend.position="none")
pBottom 
```
...
For Vent v. Plume - use 10.0 and 0.2 samples together - needed bc low number of vent sites compared to non vent sites 
**PERMANOVA**
...
AND now if station 11 is a vent ...
```{r}
...
```
```{r}
sigDino <- sigtab[sigtab$D2=="Dinoflagellata",]
dim(sigDino[sigDino$log2FoldChange>0,])
```

```{r}
sigChrys <- sigtab[sigtab$D2=="Ochrophyta",]
dim(sigChrys)
View(sigChrys)
```
```{r}
sigMAST <- sigtab[sigtab$D2=="Stramenopiles_X",]
```
